...
import '../styles/Universities-darkmode-fixes.css';
import LoginModal from '../components/LoginModal';
import SignupModal from '../components/SignupModal';
import { listStudents } from '../api/students';
import { listSupervisors } from '../api/supervisors';
import { listProjects, parseProjectRequirements } from '../api/projects';
...
const UniversitiesPage = () => {
  ...
  const [liveDetails, setLiveDetails] = useState({ students: [], supervisors: [], projects: [] });
    ...
    const universitiesData = [
      {
        ...
        image: 'https://admission.wise.edu.jo/images/auth-wise.webp',
      ...
      },
      {
        ...
        image: 'https://petra.gov.jo/upload/1750683460523.jpg',
      ...
      },
      {
        ...
        image: 'https://www.petra.gov.jo/upload/1737897069908.jpg',
      ...
      },
      {
        ...
        shortName: 'JUST',
        image: 'https://i.ytimg.com/vi/PKzQUJ8SOXw/hq720.jpg?sqp=-oaymwEhCK4FEIIDSFryq4qpAxMIARUAAAAAGAElAADIQj0AgKJD&rs=AOn4CLAsO_PWe0D-3Juvo92TPwPv0mUhnA',
      ...
      },
      {
        ...
        shortName: 'PSUT',
        image: 'https://www.psut.edu.jo/uploads/2022/05/16473481249203.jpg',
      ...
      },
      {
        ...
        image: 'https://iresizer.devops.arabiaweather.com/resize?url=https://adminassets.devops.arabiaweather.com/sites/default/files/field/image/BeFunky-collage-30.jpg&size=400x225&force_webp=1',
      ...
      },
      {
        ...
        image: 'https://tel.hu.edu.jo/img/carousel-1.jpg',
  ...
  // Load live data for expanded views (best-effort, falls back to dummy data)
  useEffect(() => {
    let isMounted = true;

    async function loadLive() {
      if (!expandedView || !showModal || !selectedUniversity) return;

      try {
        setLiveLoading(true);
        setLiveError('');

        const uniName = String(selectedUniversity?.name || '').toLowerCase();
        const uniShort = String(selectedUniversity?.shortName || '').toLowerCase();
        const matchesUniversity = (value) => {
          const v = String(value || '').toLowerCase();
          return !!v && (v === uniName || v === uniShort || v.includes(uniName) || v.includes(uniShort));
        };

        if (expandedView === 'students') {
          const students = await listStudents({ page: 1, limit: 200 }).catch(() => []);
          const filtered = (students || []).filter(s => matchesUniversity(s.university));
          ...
          if (isMounted) setLiveDetails(prev => ({ ...prev, students: mapped }));
          return;
        }

        if (expandedView === 'supervisors') {
          const supervisors = await listSupervisors().catch(() => []);
          const filtered = (supervisors || []).filter(s => matchesUniversity(s.university));
          const mapped = filtered.map((s) => ({
            name: s.name || 'Supervisor',
            title: s.specialization ? String(s.specialization) : 'Supervisor',
            expertise: s.department ? String(s.department) : '',
            bio: '',
            rating: 0
          }));

          if (isMounted) setLiveDetails(prev => ({ ...prev, supervisors: mapped }));
          return;
        }

        if (expandedView === 'projects') {
          const allProjects = await listProjects().catch(() => []);
          const filtered = (allProjects || []).filter(p => matchesUniversity(p.creator_university));

          const memberCounts = await Promise.all(
            filtered.map(p => getMembersByProject(p.id).catch(() => ({ count: 0, members: [] })))
          );

          const mapped = filtered.map((p, idx) => {
            const req = parseProjectRequirements(p?.requirements) || {};
            const tech = Array.isArray(req?.requiredSkills) ? req.requiredSkills : [];
            return {
              id: p?.id,
              name: p?.title || '',
              status: p?.status || 'planning',
              students: memberCounts[idx]?.count ?? 0,
          ...
          if (isMounted) setLiveDetails(prev => ({ ...prev, projects: mapped }));
          return;
        }
      } catch (e) {
        console.error('Failed to load university details:', e);
        if (isMounted) setLiveError(e?.response?.data?.message || e?.message || 'Failed to load data');
      } finally {
  ...
  }, [expandedView, showModal, selectedUniversity]);
      ...
      {expandedView === 'students' && showModal && selectedUniversity && (
        <div className="university-modal active">
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              {liveLoading && <div className="text-muted">Loading...</div>}
              {!liveLoading && liveError && <div className="text-danger">{liveError}</div>}
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:'1.5rem'}}>
                {(((liveDetails.students && liveDetails.students.length) ? liveDetails.students : (universityDetails[selectedUniversity.id]?.students || []))).map((student, idx) => (
        ...
        <div className="university-modal active">
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              ...
              {!liveLoading && liveError && <div className="text-danger">{liveError}</div>}
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))',gap:'1.5rem'}}>
                {(((liveDetails.supervisors && liveDetails.supervisors.length) ? liveDetails.supervisors : (universityDetails[selectedUniversity.id]?.supervisors || []))).map((sup, idx) => (
                  <div key={idx} style={{background:'#f8f9fa',padding:'1.5rem',borderRadius:16,border:'1px solid #e0e0e0'}}>
                    <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:'1rem'}}>
                      <div style={{display:'flex',alignItems:'center',gap:'1rem'}}>
                        <span style={{fontSize:'2.5rem'}}>{['👨‍🏫','👩‍🏫','🧑‍🏫'][idx%3]}</span>
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(320px,1fr))',gap:'1.5rem'}}>
                {(((liveDetails.projects && liveDetails.projects.length) ? liveDetails.projects : (universityDetails[selectedUniversity.id]?.projects || []))).map(project => (
                  <div
                    key={project.id}
                    style={{background:'#f8f9fa',padding:'1.5rem',borderRadius:16,border:'1px solid #e0e0e0'}}
                  >
