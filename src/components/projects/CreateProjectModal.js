...
const CreateProjectModal = ({ onClose, onSubmit, prefilledData }) => {
  ...
  // Prefill form data when component mounts or prefilledData changes
  useEffect(() => {
    if (prefilledData) {
      setFormData(prev => ({
        ...prev,
        ...
        field: prefilledData.field || 'IT'
      }));
    }
  }, [prefilledData]);

  const fields = ['IT', 'AI', 'Engineering', 'Business', 'Science', 'Design', 'Other'];
  const fallbackSupervisors = ['Dr. Smith', 'Dr. Johnson', 'Dr. Williams', 'Dr. Brown'];
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const rows = await listSupervisors();
        const mapped = Array.isArray(rows)
          ? rows
              .filter(s => s?.id)
              .map(s => ({ id: String(s.id), name: s?.name || 'Supervisor' }))
          : [];
        ...
        if (mapped.length) {
          ...
          setSupervisorOptions(fallbackSupervisors.map((name, idx) => ({ id: `fallback-${idx}`, name })));
        }
      } catch (e) {
        if (!isMounted) return;
        setSupervisorOptions(fallbackSupervisors.map((name, idx) => ({ id: `fallback-${idx}`, name })));
            ...
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const selected = supervisorOptions.find(o => o.id === selectedId);
                  const name = selected ? selected.name : '';
                  setFormData({
                    ...formData,
                    supervisorId: selectedId,
                    supervisorName: selectedId ? name : ''
                  });
                }}
              >
